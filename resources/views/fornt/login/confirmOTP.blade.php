<!-- https://wind-town.test/confirmOTP -->
 <!DOCTYPE html>
<html lang="vi">
<head>
    <meta charset="UTF-8">
...
<body>
    <div class="register-container">
        <div class="logo">Confirm OTP</div>
        <form action="{{ route('confirmOTP.post') }}" method="POST" >
            @csrf

            @if ($errors->has('confirmOTP_error'))
            <div style="color: red; margin-bottom: 10px;">
            {{ $errors->first('confirmOTP_error') }}
            </div>
            @endif
            
            <div class="input-group" style="color: #00fff2;">
                <span>Enter the OTP code sent to your email address or phone number</span>
            </div>
            <div class="input-group">
                <input type="text" class="input-field" name="OTP" placeholder="OTP Code">
            </div>
